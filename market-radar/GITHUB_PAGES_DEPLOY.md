...
3. `main` 브랜치에 푸시하거나 Actions 탭에서 `Deploy Market Radar Pages`를 수동 실행합니다.
4. 배포가 끝나면 Pages의 `Visit site` 링크로 접속합니다.

## 현재 workflow

`.github/workflows/market-radar-pages.yml`은 `build-github-pages-artifact.sh`로 `_site` 폴더를 만들고 앱 실행에 필요한 정적 파일만 아티팩트로 올립니다.

- Docker를 사용하지 않습니다.
- GitHub Actions에서 바로 배포합니다.
- `market-radar/index.html`이 사이트 첫 화면이 됩니다.
- `.nojekyll`을 포함해 GitHub Pages가 파일을 그대로 서빙하도록 합니다.
- 배포 아티팩트에는 `index.html`, `health.html`, `.nojekyll`, `src/app.mjs`, `src/data.mjs`, `src/styles.css`만 들어갑니다.
